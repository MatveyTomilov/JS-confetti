...
function initConfetti() {
  // Добавляем контейнер для анимации в конец тега <body>, если его еще нет
  if (!$('#confetti').length) {
    $('body').append('<div id="confetti"></div>');
  }

  // Добавляем стили для контейнера, если они еще не добавлены
  ...
  $(document).ready(function() {
    ...
    // Определение класса ConfettiPaper
    ...
    // Определение класса ConfettiRibbon
    ...
    // Создаем контекст конфетти
    var confetti = {};
    confetti.Context = function(parentId) {
      var i = 0;
      ...
      // Инициализируем бумажки конфетти
      for (i = 0; i < 25; i++) {
    ...
    // Инициализируем анимацию конфетти
    var confettiContext = new confetti.Context('confetti');
    console.log('Confetti animation started');
    confettiContext.start();

    // Обработка изменения размера окна
    $(window).resize(function() {
      confettiContext.resize();
    });
  });
}
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = confettiModule;
}

// Экспорт по умолчанию для ES6 модулей
export default confettiModule;
